import { Col, Descriptions, Divider, InputNumber, Row, Space, Tag, Typography } from "antd";
...
export function PricingProductSummary({
    ...
    return (
      <ModuleSectionCard>
        <div className="module-table-shell">
          <ModuleEmptyState
            title="Nenhum produto disponivel para analise"
            description="A lista filtrada nao retornou itens. Ajuste o recorte da pagina para recuperar produtos com base de precificacao."
          />
        </div>
      </ModuleSectionCard>
    );
  ...
  return (
    <Space direction="vertical" size={16} style={{ width: "100%" }}>
      <ModuleDetailPanel
        eyebrow="Painel secundario"
        title={productView.product.name}
        description={`${productView.product.presentation || "Sem apresentacao"} · ${productView.product.category || "Sem categoria"}`}
      >
      ...
      <ModuleAlertStack
        title="Leitura parcial da precificacao"
        items={[...productView.notes, ...productView.hybridCost.notes]}
        ...
        <div className="module-table-shell">
          <Space direction="vertical" size={12} style={{ width: "100%" }}>
            ...
            {productView.breakdown.compositionItems.length ? (
              ...
              <ModuleEmptyState
                title="Nenhum item de composicao encontrado"
                description="Este produto ainda nao possui receita detalhada suficiente para abrir o custo teorico por insumo."
              />
            )}
          </Space>
        </div>
